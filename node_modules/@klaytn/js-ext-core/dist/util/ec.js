...
function getCompressedPublicKey(pub) {
    if (lodash_1.default.isString(pub)) { // Hex string
        const hex = data_1.HexStr.from(data_1.HexStr.withHexPrefix(pub));
        if (data_1.HexStr.isHex(hex, 33) || data_1.HexStr.isHex(hex, 65)) {
            const serialized = data_1.HexStr.stripHexPrefix(hex);
            const pubkey = secp256k1.keyFromPublic(serialized, "hex");
            return "0x" + pubkey.getPublic(true, "hex");
        }
    }
    if (lodash_1.default.isString(pub.x) && lodash_1.default.isString(pub.y)) {
        pub.x = data_1.HexStr.stripHexPrefix(pub.x);
        pub.y = data_1.HexStr.stripHexPrefix(pub.y);
